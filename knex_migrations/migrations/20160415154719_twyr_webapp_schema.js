...
exports.up = function(knex, Promise) {
	...
	// Step 15: Setup user-defined functions on Component Widget Permissions table
	.then(function() {
		return Promise.all([
			knex.schema.withSchema('public')
			.raw(
	...
	.then(function() {
		return Promise.all([
			...
			knex.schema.withSchema('public').raw('CREATE TRIGGER trigger_check_component_widget_permission_upsert_is_valid BEFORE INSERT OR UPDATE ON public.component_widgets_permissions FOR EACH ROW EXECUTE PROCEDURE public.fn_check_component_widget_permission_upsert_is_valid();')
		]);
	});
};
